return { -- Fuzzy Finder (files, lsp, etc)
  ...
  config = function()
    ...
    local find_files_fn = function()
      builtin.find_files({
        hidden = true,
        find_command = find_command,
      })
    end
    vim.keymap.set("n", "<leader>sf", find_files_fn, { desc = "[S]earch [F]iles (including hidden)" })
    vim.keymap.set("n", "<C-p>", find_files_fn, { desc = "Find files" })
